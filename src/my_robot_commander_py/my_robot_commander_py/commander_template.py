...
class CommanderNode(Node):
    def __init__(self):
        ...
        self.arm_: PlanningComponent = self.robot.get_planning_component("arm")
        self.gripper_: PlanningComponent = self.robot.get_planning_component("gripper")

        self.open_gripper_sub_ = self.create_subscription(Bool, "open_gripper", self.open_gripper_callback, 10)

        self.joint_cmd_sub_ = self.create_subscription(Float64MultiArray, "joint_command", self.joint_command_callback, 10)

        self.pose_cmd_sub_ = self.create_subscription(PoseCommand, "pose_command", self.pose_command_callback, 10)
    ...
    def go_to_joints_target(self, joints):
        ...
        joint_values = {
            "joint1_base_shoulder": joints[0],
            "joint2_shoulder_arm": joints[1],
            "joint3_arm_elbow": joints[2],
            "joint4_elbow_forearm": joints[3],
            "joint5_forearm_wrist": joints[4],
            "joint6_wrist_hand": joints[5]
        }
    ...
    def go_to_pose_target(self, x, y, z, roll, pitch, yaw):
        q_x, q_y, q_z, q_w = tft.quaternion_from_euler(roll, pitch, yaw)
